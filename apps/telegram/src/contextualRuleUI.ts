...
export function openContextualRuleUI(host:HTMLElement,platform:PlatformAdapter,game:TelegramBackendGame,onChanged:()=>void,setStatus:StatusSetter):boolean{closeContextualRuleUI(host);const action=contextualAction(game.getState(),game);if(!action)return false;const wrapper=document.createElement('div');wrapper.className='tg-context-layer';wrapper.dataset.contextLayer='active';wrapper.innerHTML=`<button class="tg-context-scrim" type="button" data-context-scrim aria-label="Close contextual panel"></button><section class="tg-context-sheet" role="dialog" aria-modal="true" aria-labelledby="tgContextTitle"><div class="tg-context-handle" aria-hidden="true"></div><header class="tg-context-header"><div><small>ACTIVE GAME ACTION</small><h2 id="tgContextTitle">${escapeHTML(action.title)}</h2></div><button type="button" class="tg-context-close" data-context-close aria-label="Close">×</button></header>${action.body}<p class="tg-context-authority">Shared game state decides whether each action is legal and what happens next.</p></section>`;host.append(wrapper);const close=()=>closeContextualRuleUI(host);wrapper.querySelector<HTMLElement>('[data-context-scrim]')?.addEventListener('click',close);wrapper.querySelector<HTMLElement>('[data-context-close]')?.addEventListener('click',close);wrapper.querySelectorAll<HTMLButtonElement>('[data-command]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;const result=await runCommand(button,wrapper,game);platform.haptic(result.ok?'medium':'light');setStatus(result.ok?{text:result.success??'Game state updated.',tone:'success'}:{text:result.error??'The game rejected that action.',tone:'warning'});close();onChanged();}));platform.haptic('light');return true;}
...
function contextualAction(state:GameState,game:TelegramBackendGame):ContextAction|null{
  ...
  const step=social.canonicalStep;if(!step)return null;
  ...
  if(step==='PARANOIA_CLASSIC_ANSWER'&&social.pendingTargetId===humanId)return{title:'Choose Answer Player',body:targetGrid(game,state.players.filter(p=>p.id!==humanId).map(p=>p.id),'SELECT_PARANOIA_CLASSIC_ANSWER')};
  ...
  if(step==='DUEL_INITIATOR'&&social.pendingDuel?.initiatorId===humanId)return duelResponse('initiator',social.prompt?.text??'Complete the Duel response.');
  if(step==='DUEL_OPPONENT'&&social.pendingDuel?.opponentId===humanId)return duelResponse('opponent',social.prompt?.text??'Complete the Duel response.');
  if(step==='DUEL_VOTE'&&social.pendingDuel?.vote?.eligibleVoterIds.includes(humanId)&&!social.pendingDuel.vote.votes[humanId])return{title:'Duel Group Vote',body:targetGrid(game,[social.pendingDuel.initiatorId,social.pendingDuel.opponentId!],'DUEL_VOTE')};
...
function duelResponse(side:'initiator'|'opponent',prompt:string):ContextAction{return{title:'Your Duel Response',body:`${promptCard('DUEL',prompt)}${textArea('duel-response','Your response')}${commandButton('SUBMIT_DUEL_TYPED','Submit Response',side)}${commandButton('SUBMIT_DUEL_LIVE','Answered Live',side)}`};}
function truthDareAnswer(title:string,prompt:string,options:readonly string[]|undefined,mode:AnswerMode|null):ContextAction{if(options?.length)return{title,body:`${promptCard(title.toUpperCase(),prompt)}<div class="tg-answer-grid">${options.map(option=>commandButton('SUBMIT_CHOICE',option,option)).join('')}</div>${commandButton('PASS_PROMPT','Pass / Not For Me','')}`};if(mode==='TYPE')return{title,body:`${promptCard(title.toUpperCase(),prompt)}${textArea('context-answer','Your answer')}${commandButton('SUBMIT_TYPED_ANSWER','Submit Answer','')}${commandButton('PASS_PROMPT','Pass / Not For Me','')}`};return{title,body:`${promptCard(title.toUpperCase(),prompt)}<div class="tg-answer-grid">${commandButton('ANSWERED_LIVE_NOW','Answered Live','')}${commandButton('SELECT_ANSWER_MODE','Type','TYPE')}</div>${commandButton('PASS_PROMPT','Pass / Not For Me','')}`};}
...
async function runCommand(button:HTMLButtonElement,wrapper:HTMLElement,game:TelegramBackendGame):Promise<ContextResult>{
  const command=button.dataset.command??'',value=button.dataset.value??'';const send=async(payload:Parameters<TelegramBackendGame['send']>[0]):Promise<ContextResult>=>{const result=await game.send(payload);return result.ok?{ok:true}:{ok:false,error:result.error?.message??'The game rejected that action.'};};
