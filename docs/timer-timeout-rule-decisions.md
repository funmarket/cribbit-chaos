...
| Timer identity | Phase 3C brief | The live timer slot is authoritative. `purpose`, `ownerPlayerId`, `startedAt`, and `deadlineAt` describe the active timer; `startedAtRevision` is retained as committed-state metadata, but intermediate social-state revisions do not by themselves invalidate a still-active social timer. | Internal state only | No content | No | HIGH | None |
| Timeout commands | Phase 3C brief and existing command contract | The reducer handles `TIMEOUT_TURN` and `TIMEOUT_SOCIAL` as authoritative timeout commands. The command actor id remains a provisional server-actor convention because the existing command envelope is player-scoped; authorization still belongs to the server. | Public event surface is minimal. | No content | No | HIGH | A future transport contract may add a dedicated system actor if needed |
...
| Stale timeout | Phase 3C brief | If the live timer no longer matches the authoritative interaction, the command fails with `STALE_TIMEOUT`. Rewind, pass, answer completion, duel response resolution, or any explicit timer replacement invalidate the old timer; merely advancing through a live social interaction does not. | Private failure only | No content | No | HIGH | None |
