...
- `game_commands.command_id` is `uuid` and is the **global** primary key/idempotency key of the command log. The API takes a transaction-scoped advisory lock derived from the UUID before duplicate lookup, preventing simultaneous retries/collisions from racing into the primary-key constraint. The persisted `payload` is compared with the incoming command through the same exhaustive `fingerprintGameCommand` used by the shared engine. Same UUID + same semantic fingerprint replays the stored result; same UUID + different session/player/type/payload returns controlled `COMMAND_ID_COLLISION` without a second gameplay mutation. `commandId` and `expected_revision` are envelope/execution metadata and are intentionally excluded from semantic identity. `command_type` is `text`, `payload`/`result` are `jsonb`, `expected_revision` is `bigint`, `session_id`/`actor_user_id` are `uuid`.
...
- Schema migrations are now also exercised in GitHub CI against PostgreSQL 16 before `npm test`. RECOVERY-HARDEN-3B verifies command-log replay/collision behavior with real DB-backed tests: sequential cross-session UUID reuse returns controlled `COMMAND_ID_COLLISION`; concurrent identical retries mutate once and persist one row; concurrent cross-session UUID reuse yields one success, one controlled collision and exactly one global command row.
