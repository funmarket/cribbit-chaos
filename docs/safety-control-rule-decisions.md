...
| --- | --- | --- | --- | --- | --- | --- |
| PASS | Phase 3 brief, approved V4 control language, and locked Truth/Dare refusal rule | `PASS_PROMPT` is an authoritative control for active eligible social prompts. Truth and Dare Pass / Not for Me draws exactly 2 real cards for the refusing player before the social effect resolves, so the normal win check cannot run until after the penalty is applied. Paranoia resolves privately for the triggering player before target selection; Duel lets the initiator Pass before target selection and then lets either actual Duel participant Pass after target selection; Chaos targeting `all` records each required participant independently and resolves only after the full participant set completes. Duel pass uses the authoritative social-turn boundary and does not invent extra winner/scoring semantics. | Pass acknowledgments are `PLAYER_PRIVATE`. Truth/Dare penalty draw metadata is private to the refusing player. Resolution events remain public and minimal. | No | HIGH | Duel scoring / forfeit consequences remain deferred |
...
- Canonical: Pass is private; Rewind is once per session for eligible Truth/Dare prompts; Flag is a private moderation signal that only includes actual participants; none of these are cards.
- Provisional: the precise UX surface for these controls in the clients.
- Deferred: Duel scoring/forfeit consequences beyond the safe refusal path, moderation persistence, and all Phase 3B answer-mode behavior.
